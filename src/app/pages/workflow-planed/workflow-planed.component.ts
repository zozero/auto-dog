...
@Component({
  ...
  imports: [LayoutModule, CommonModule, ProjectMenusComponent],
})
export class WorkflowPlanedComponent implements OnInit {
  ...
  test2() {
    ...
    let edgesMap:any = new Map();
    let curShowNodesMap:any = new Map();
    let highlighting:any = false;
    let currentFocus:any;
    const width:any = this.document.getElementById('container')?.scrollWidth;
    const height:any = this.document.getElementById('container')?.scrollHeight || 500;

    const LIMIT_OVERFLOW_WIDTH:any = width;
    const LIMIT_OVERFLOW_HEIGHT:any = height;

    const mapNodeSize:any = (nodes:any, propertyName:any, visualRange:any) => {
      let minp = 9999999999;
      let maxp = -9999999999;
    ...
    G6.registerBehavior('double-finger-drag-canvas', {
      ...
      onWheel: (ev:any) => {
        if (ev.ctrlKey) {
          ...
          const x:any = ev.deltaX || ev.movementX;
          const y:any = ev.deltaY || ev.movementY || (-ev.wheelDelta * 125) / 3;
    ...
    void fetch('https://gw.alipayobjects.com/os/bmw-prod/fc6e64fc-be94-40fb-b9e2-2d13dd414f38.json')
      .then((res) => res.json())
      .then((data) => {
        loadData(data);
      });
    ...
    graph.on('node:click', (e:any) => {
      ...
      if (model.level === 0) {
        ...
        if (currentFocus && currentFocus.id === model.id) {
          ...
          curShowNodes.forEach((nu:any, i:any) => {
            const lu = nu.level;
            curShowNodes.forEach((nv:any, j:any) => {
              if (j <= i) return;
              const lv = nv.level;
              let edgeId;
              if (lu < lv) {
    ...
    if (typeof window !== 'undefined')
      window.onresize = () => {
        ...
        if (!this.document.getElementById('container') || !this.document.getElementById('container')?.scrollWidth || !this.document.getElementById('container')?.scrollHeight) return;
        graph.changeSize(this.document.getElementById('container')?.scrollWidth, this.document.getElementById('container')?.scrollHeight);
      };
